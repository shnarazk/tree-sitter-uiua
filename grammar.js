...
module.exports = grammar({
  ...
  rules: {
    ...
    PROGRAM: ($) =>
      seq(
        repeat(choice(seq(choice($.binding, $.segment, $.module), $._endOfLine), $.emptyMultiLineString)),
        choice($.binding, $.segment, $.module, $.emptyMultiLineString),
        optional($._endOfLine),
      ),
    binding: ($) => seq($.identifier, $.leftArrow, optional($.signature), choice($.inlineFunction, $.segment)),
    module: ($) => seq($.tripleMinus, repeat(seq($.segment, $._endOfLine)), $.segment, $._endOfLine, $.tripleMinus),
    segment: ($) => prec.right(seq(choice($.term, $.comment), optional($.segment))),
    term: ($) =>
      choice(
        $.switchFunctions,
    ...
    number: ($) => choice($.constant, $.fraction, token(choice(/¯?[πητ]([eE]¯?\d+)?/, /¯?\d+(\.\d+)?([eE]¯?\d+)?/))),
    fraction: ($) => token(/¯?\d+\/\d+/),
    otherConstant: ($) =>
      choice(
        token("Days"),
    ...
    character: ($) =>
      prec(5, token(/@([^\\]|\\[Wbnrst0\\"'_]|\\x[0-9A-Fa-f]{2,2}|\\u[0-9A-Fa-f]{4,4}|\\u\{[0-9A-Fa-f]+\})/)),
    string: ($) => token(/\$?"(\\["bnrst0]||[^"])+"/),
    multiLineString: ($) => /\$ .*/,
    ...
    compound: ($) =>
      choice(
        prec(1, seq($.modifier1, choice($.inlineFunction, $.function, $.system, $.identifier))),
        prec(1, seq($.modifier2, choice($.inlineFunction, $.function, $.system, $.identifier))),
        prec(
          2,
          seq(
            $.modifier2,
